        ...
        with self._isolated_import():
            try:
                ...
                if not(use_gpu and torch.cuda.is_available()):
                    ...
                    print("[INFO] Aplicado parche de compatibilidad CPU a RootNet")


                model.load_state_dict(state_dict)
                
                self.model = model.eval()
                print("[INFO] RootNet cargado exitosamente")
                
            except Exception as e:
                print(f"[ERROR] No se pudo cargar RootNet: {e}")
                import traceback
                traceback.print_exc()
                self.model = None
        ...
        try:
            with self._isolated_import():
                ...
                if processed_bbox is None:
                    print(f"[WARNING] processed_bbox is None for bbox: {bbox}. Using fallback depth.")
                    return self._fallback_depth(bbox)
            ...
            print(f"[WARNING] Error en predicción RootNet: {e}")
            import traceback
            traceback.print_exc()
            return self._fallback_depth(bbox)
